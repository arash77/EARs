...
def adjust_score(reviewer, tags):
    ...
    if reviewer["Last Review"] == "NA":
        score += 50
    if "ERGA-BGE" in tags and reviewer["Institution"] in [
        "CNAG",
        "Sanger",
        "Genoscope",
        "SciLifeLab",
...
def print_csv(data_list, selected_reviewer):
    ...
    col_widths = {
        header: max(
            len(header),
            max((len(str(row[header])) for row in data_list), default=len(header)),
        )
    ...
    if selected_reviewer:
        data_list = [selected_reviewer] + [
            d for d in data_list if d != selected_reviewer
        ]

    for data in data_list:
        data_row = " | ".join(
            str(data[header]).ljust(col_widths[header]) for header in headers
        )
        print(data_row)
...
def select_best_reviewer(data, calling_institution, use_bge):
    eligible_candidates = [
        ...
        if reviewer["Active"] == "Y"
        and reviewer["Busy"] == "N"
        and reviewer["Institution"] != calling_institution
    ]

    eligible_candidates.sort(
        key=lambda x: (
            -x["Adjusted Score"],
            x["Parsed Last Review"],
            x["Total Reviews"],
        )
    )

    top_score = (
        eligible_candidates[0]["Adjusted Score"] if eligible_candidates else None
    )
    top_candidates = (
        [c for c in eligible_candidates if c["Adjusted Score"] == top_score]
    ...
    final_candidates = [
        ...
        if c["Parsed Last Review"] == oldest_review
        and c["Total Reviews"] == fewest_reviews
    ]
...
def select_random_supervisor(data, exclude_id):
    supervisors = [
        reviewer
        for reviewer in data
        if reviewer["Supervisor"] == "Y" and reviewer["Github ID"] != exclude_id
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Select a candidate for EAR reviewing. Also can select a supervisor."
    )
    parser.add_argument(
        "-i",
        "--institution",
        required=True,
        help="Institution of the person requesting the review.",
    )
    parser.add_argument(
        "-t",
        "--tag",
        nargs="*",
        default=[],
        help="Specify one or more tags to influence selection criteria, such as 'ERGA-BGE' to favor reviewers from the BGE project.",
    )
    parser.add_argument(
        "-u",
        "--user",
        help="Github user ID of the person requesting the review. Only required for selecting a supervisor.",
    )
    parser.add_argument(
        "-s",
        "--supervisor",
        action="store_true",
        help="Flag to select a supervisor randomly.",
    )
    parser.add_argument(
        "-v",
        "--version",
    ...
    if csv_data:
        ...
        if args.supervisor:
            if not args.user:
                print(
                    "Github user ID must be provided with --user when using --supervisor."
                )
                return
            selected_supervisor = select_random_supervisor(data, args.user)
            if selected_supervisor:
                print(
                    f"Selected supervisor: {selected_supervisor['Full Name']} ({selected_supervisor['Github ID']})"
                )
            else:
                print("No eligible supervisors found.")
            return

        all_eligible_candidates, top_candidates, selection_reason = (
            select_best_reviewer(data, args.institution, args.tag)
        )
        print("*****")
